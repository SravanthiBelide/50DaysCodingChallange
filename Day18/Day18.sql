...
SELECT c.comment_text, u.username, c.post_id
FROM Comments c
...
FROM Likes
GROUP BY post_id
HAVING COUNT(like_id) > (
    SELECT COUNT(like_id) / COUNT(DISTINCT post_id) 
    FROM Likes
);
...
WHERE u.user_id NOT IN (SELECT DISTINCT user_id FROM Posts);

SELECT u.username
FROM Users u
JOIN Friendships f ON (u.user_id = f.user_id1 OR u.user_id = f.user_id2)
WHERE (f.user_id1 = 3 OR f.user_id2 = 3) AND u.user_id != 3;

SELECT DISTINCT p.content
FROM Posts p
JOIN Likes l ON p.post_id = l.post_id
WHERE l.user_id IN (
    SELECT CASE WHEN user_id1 = 3 THEN user_id2 ELSE user_id1 END
    FROM Friendships
...
DELIMITER //
CREATE PROCEDURE GetUserActivity(IN target_user_id INT)
BEGIN
    SELECT 
        (SELECT COUNT(*) FROM Posts WHERE user_id = target_user_id) AS total_posts,
        (SELECT COUNT(*) FROM Likes WHERE user_id = target_user_id) AS likes_given,
        (SELECT COUNT(*) FROM Likes l JOIN Posts p ON l.post_id = p.post_id 
         WHERE p.user_id = target_user_id) AS likes_received,
        (SELECT COUNT(*) FROM Comments WHERE user_id = target_user_id) AS comments_made;
END //
DELIMITER ;
